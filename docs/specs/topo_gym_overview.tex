...
\begin{itemize}[leftmargin=2em, itemsep=3pt]
  \item \texttt{TopoGym/IceShip}. Arctic ship navigation (the agent
        is the sailboat). Coastal ice sheets attach to the north and
        south edges --- land masses, not floating obstacles ---
        octagonal sealed bergs float in the open water as decoys, and
        the treasure sits in a cavity deep inside the large eastern
        sheet, reachable \emph{only} through a guaranteed narrow
        width-1 channel threaded across the ice (the guarantee is
        unit-tested: blocking the channel disconnects the goal).
        Ice-adjacent water cells carry the directional adjacency
        flags (a doorway reads as ice on both sides); open-water
        cells carry the water feature only. Texture makes berg
        avoidance cheap on sight; whether the channel leads anywhere
        remains invisible to any local feature.
  ...
  \item \texttt{TopoGym/ClownChase}. The reward-deception scenario.
        Three sealed decoys cluster on one side of the map with a
        clown NPC random-walking among them; every agent step that
        reduces the distance to the clown pays a tiny reward
        ($10^{-3}$) from a budget of $2.0$ that spans the agent's
        lifetime on the layout --- the distractor runs out after a
        few thousand rewarding steps in total. The treasure chamber
        sits on the opposite side (the construction rejects layouts
        where the sides are not genuinely opposite). Texture
        distinguishes doors, floor, room interiors, the treasure
        cell, and clown proximity (a slot lights up when the clown is
        within one cell); the clown's position is otherwise sensed
        only through the reward gradient --- which is exactly the
        signal that must be distrusted.
  \item \texttt{TopoGym/EnvironmentalIceShip}. IceShip with seasons.
        Each episode is drawn sunny (summer) or snowing (winter), with
        matching texture palettes, and the water texture's value
        encodes the season (1.0 sunny, 0.5 cold). In winter the ice
        \emph{grows}: from step 40, the narrow channel freezes shut
        one cell every 6 steps from its mouth toward the door ---
        standing on a cell when it freezes, or being inside the
        treasure cavity when the channel closes, ends the episode. In
        summer the ice shrinks: the channel's flanking walls melt on
        the same schedule, widening the passage. The certified
        metadata describes the episode-start geometry; a
        \texttt{season} make-kwarg forces either season for
        controlled evaluation.
  \item \texttt{TopoGym/SearchRescue}. Search and rescue. A person is
        trapped in one large open chamber buried in a dense field of
        shrapnel (26 small transient obstacles, each a small
        $\Hm{1}$ class of its own). The victim's chamber is the only
        \emph{large, persistent} hole: in the agent's own discovery
        filtration --- the archive --- shrapnel loops are born small
        and die quickly as their shards are circumnavigated, while the
        chamber's enclosing class grows large and refuses to die.
        Persistence, not luck, finds the person; certified
        $b_1 = 27$.
\end{itemize}
