...
(future families extend to v2 without altering v1 entries). The
experiment program that consumes TopoGym-v1 is specified in
\texttt{papers/topoexplore/experiment\_design.tex} (campaign X1);
this document is the authority on the environments themselves. GridWorld2D is specified fully here; the Texture and Top
...
the agent in place. Standing conventions, inherited from the theory
document and enforced at generation time:
\begin{itemize}[leftmargin=2em, itemsep=1pt]
  ...
  \item a chamber is an obstacle wall enclosing free interior once
        ...
        construction, which is what makes it a decoy from the
...
\begin{itemize}[leftmargin=2em, itemsep=2pt]
  ...
  \item \texttt{nested}: \texttt{depth} concentric shells around one
        ...
        fails, and where learned-persistence baselines are predicted
        to win. Bars are born sequentially: entering shell $j$ first
...
manifest plus fixed seed lists ---
tuning seeds and evaluation seeds, identical environments, disjoint
seeds --- as specified in the experiment design.

\subsection{Ground truth, certification, and instrumentation}

Read-only accessors per instance: the wall mask; the chamber list
with interiors $I(W_i)$, perimeters $P_i$, door cells $D(W_i)$; the
decoy list with representative rims; per-cell annotations (chamber
and door membership; full-graph Ollivier--Ricci curvature, computed
once by the generator); and exact snapshot save/restore. A
certification checker validates a reported class list against the
decoy ground truth and each certificate (every rim action tried
$u_{\max}$ times without discovery, or explicit interior filling)
directly from the run log. The shared recorder writes the full metric
catalogue of the experiment design to Parquet at four granularities
(per step, per epoch, per training run, per evaluation run).
...
\texttt{reward\_mode} configuration field:
\begin{itemize}[leftmargin=2em, itemsep=1pt]
  \item \texttt{none} (default) --- no extrinsic reward; the
        environment is a pure-exploration benchmark, and event-based
        metrics (entry times, coverage) carry the evaluation.
  \item \texttt{sparse} --- $+1$ terminal reward on reaching a target
        cell placed inside a designated chamber (in Texture
        scenarios, the treasure-chest cell). Placement inside a
        chamber makes steps-to-first-reward coincide with
...
reward-consuming components (the hole-value and frontier heads, the
planner's exploitation term), and for deception studies; the
TopoExplore experiment program runs \texttt{none} throughout to keep
an extra variable out of its main results.
...
right. Moving into an obstacle leaves the agent in place. With
...
edge applies the identification map (coordinate and orientation
reversal where the identification is flipped) --- the action space
itself is unchanged. Family-specific cell mechanics (hazard
...
\paragraph{SpaceWarp and the certificate (semantics, not
soundness).} SpaceWarp does not break the certificate --- it sharpens
what the certificate says. Both certified statements remain true
here: the treasure chamber's enclosing class is genuinely permanent
in the spatial complex (its wall has no door, so the class never dies
even after wormhole entry archives the interior --- the wall stays
uncovered and the hugging cycle never bounds), and its gate log is
genuinely exhausted (no local entry exists). What the environment
changes is the gloss: a decoy certificate certifies no local entry
and a permanent spatial $\Hm{1}$ class, not unenterability --- with
non-local transitions, certified decoys can still be enterable
through a wormhole. SpaceWarp is included to make that semantic
boundary measurable; the extension that upgrades the certificate from
``no local entry'' to ``no entry'' --- evaluating gate evidence and
the complex on the archive transition graph, where wormhole edges
appear once traversed --- is future work the environment is built to
test.
...
complex construction (the Rips complex on the quotient metric), the
interaction of wrap-around with the archive and with return-by-replay,
detection of the ambient classes versus feature classes, and the
per-topology registry entries are to be specified in a forthcoming
