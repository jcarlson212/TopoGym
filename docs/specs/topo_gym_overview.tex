...
next query rebuilds. Measured on the cubical backend over a dense
visited region:
...
10{,}000  & 0.26\,s & 0.24\,s & 0.73\,s  & $\sim$0 \\
50{,}000  & 1.40\,s & 1.57\,s & 12.7\,s  & $\sim$0 \\
100{,}000 & 3.04\,s & 5.55\,s & 43.6\,s  & 0.01\,s \\
\bottomrule
\end{tabular}
\end{center}

\noindent The build and rims are linear in the archive, \texttt{betti}
near-linear, and \texttt{representatives} superlinear --- comfortable
to roughly 20{,}000 cells and expensive past 50{,}000. The costs are
sequential, so extracting cycles from a 100{,}000-cell archive is the
build plus the extraction, about 47\,s in total. For scale, only
\texttt{Chambers2-400} has more free cells than that; a 50-grid
archive is around 2{,}500 cells, where the same call takes
$0.02$\,s. Query once an episode rather than once a step, and treat
\texttt{torsion} --- an integer Smith normal form, minutes at
20{,}000 cells --- as an offline diagnostic.
...
\texttt{docs/manifest.csv}, one row per instance with its canonical
configuration, certified topology, turn-aware optimal $d^{\star}$,
and horizon. Recording $d^{\star}$ is what makes the splits
\emph{checkable}: anyone can confirm that train, val, and test have
comparable difficulty distributions rather than taking it on faith.
...
extrapolation} (train at the smaller sizes, test at 200 and 400 ---
the \texttt{Chambers2} family exists for precisely this) and
\textbf{family hold-out}, where whole families are withheld to
measure concept transfer.
