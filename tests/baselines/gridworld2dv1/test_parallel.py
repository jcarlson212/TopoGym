...
def test_serial_fallback_when_the_pool_cannot_start(monkeypatch):
    """A pool can fail for reasons unrelated to the work. Losing it
    should cost speed, not the evaluation."""
    from concurrent.futures.process import BrokenProcessPool

    import topogym.baselines.gridworld2dv1.parallel as parallel
    ...
    def explode(*args, **kwargs):
        raise BrokenProcessPool("handle is closed")

    monkeypatch.setattr("concurrent.futures.ProcessPoolExecutor",
                        explode)
    assert parallel.map_instances(Doubler(), [1, 2, 3], workers=4) == \
        [2, 4, 6]
